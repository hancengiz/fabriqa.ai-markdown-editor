...
export const livePreviewPlugin = ViewPlugin.fromClass(
  class {
      ...
      switch (type.name) {
        ...
        case 'HorizontalRule':
          ...
          if (!decoratedRanges.has(hrKey)) {
            ...
            // Replace the HR with the widget
            decorations.push(
              Decoration.replace({
     ...
     * Renders tables as HTML in live preview
     * Obsidian-style: Shows raw markdown when cursor is inside the table
     */
    handleTable(tableNode: SyntaxNode, view: EditorView, decorations: Range<Decoration>[], decoratedRanges: Set<string>): void {
      ...
      // We use the same approach as Mermaid diagrams since tables are multi-line
